    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-500">
      ...
      <div className="lg:col-span-2 space-y-6">
         <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Интеллектуальный КПП</h2>
         ...
         <div className="grid grid-cols-2 gap-4 h-96">
            {['GATE-1 (Въезд)', 'GATE-2 (Въезд)', 'GATE-3 (Выезд)', 'GATE-4 (Выезд)'].map((cam, idx) => (
              <div key={idx} className="relative bg-black rounded-lg overflow-hidden border border-slate-200 dark:border-slate-800 group shadow-sm">
                 {/* Mock Camera UI */}
                 ...
                 <div className="w-full h-full bg-slate-100 dark:bg-slate-900 flex items-center justify-center relative">
                    <Truck size={48} className="text-slate-300 dark:text-slate-800 group-hover:text-slate-400 dark:group-hover:text-slate-700 transition-colors" />
                    <div className="absolute inset-0 border-2 border-slate-300/50 dark:border-slate-800/50 m-4 rounded pointer-events-none">
         ...
         <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
           <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Журнал доступа (24ч)</h3>
           <table className="w-full text-left text-sm">
             ...
             <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
               {gates.map(gate => (
                 <tr key={gate.id} className="group hover:bg-slate-50 dark:hover:bg-slate-900/50">
                   <td className="py-3 font-mono text-slate-500 dark:text-slate-400">{gate.timestamp}</td>
                   <td className="py-3 font-bold text-slate-800 dark:text-slate-200">{gate.truckPlate}</td>
                   <td className="py-3 text-slate-600 dark:text-slate-300">{gate.driverName}</td>
                   <td className="py-3 text-slate-500 dark:text-slate-400">{gate.company}</td>
                   <td className="py-3 text-slate-500 dark:text-slate-400">{gate.gateId}</td>
                   <td className="py-3">
      ...
      <div className="space-y-6">
        <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
          <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Ручное управление</h3>
          <div className="space-y-4">
            <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-500/30 rounded-lg">
               <div className="flex justify-between items-start mb-2">
                 ...
                 <span className="text-slate-500 text-xs">GATE-2</span>
               </div>
               <div className="text-center py-4">
                 <p className="text-2xl font-bold text-slate-900 dark:text-white mb-1">B 889 BC 02</p>
                 <p className="text-sm text-slate-500 dark:text-slate-400">SilkRoad Logistics</p>
               </div>
               <div className="grid grid-cols-2 gap-2 mt-2">
                 ...
                 <button className="py-2 bg-slate-200 dark:bg-slate-800 hover:bg-red-600 hover:text-white text-slate-700 dark:text-slate-300 rounded text-sm font-medium transition-colors">
                   Отказать
                 </button>
               </div>
            </div>
            ...
            <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800">
              <span className="text-sm text-slate-600 dark:text-slate-300">Голосовая связь (КПП-2)</span>
              <button className="p-2 bg-slate-200 dark:bg-slate-800 hover:bg-blue-600 rounded-full text-slate-500 dark:text-slate-400 hover:text-white transition-colors">
                <Mic size={16} />
              </button>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm">
           <h3 className="font-semibold text-slate-900 dark:text-white mb-4">Статистика за смену</h3>
           <div className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-500 dark:text-slate-400">Всего проездов</span>
                <span className="text-lg font-bold text-slate-900 dark:text-white">142</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-500 dark:text-slate-400">Среднее время обр.</span>
                <span className="text-lg font-bold text-slate-900 dark:text-white">4.2 мин</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm text-slate-500 dark:text-slate-400">Отказов</span>
